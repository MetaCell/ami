...
export default class ParsersNifti extends ParsersVolume {
  ...
  private _niftiHeader: any;
  private _niftiImage: ArrayBuffer | null;
  private _ordered: boolean;
  private _orderedData: any;

  private _qfac: number;
  ...
  dimensionIndexValues(frameIndex: number = 0): null {
    return null;
  }

  instanceNumber(frameIndex: number = 0): number {
    return frameIndex;
  }

  windowCenter(frameIndex: number = 0): null {
    // calc min and calc max
    return null;
  }

  windowWidth(frameIndex: number = 0): null {
    // calc min and calc max
    return null;
  }
