...
interface OrderedPoint extends THREE.Vector3 {
...
const widgetsFreehand = (three = window.THREE) => {
  ...
  return class Widgetsfreehand extends Constructor {
    ...
    updateMesh() {
      ...
      for (let j = 0; j < points.length; j++) {
        const point = new three.Vector3(points[j].x, points[j].y, points[j].z) as OrderedPoint;
...
export default widgetsFreehand(THREE);
