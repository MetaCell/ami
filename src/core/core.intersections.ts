...
export default class Intersections {
   ...
   *
   * @returns {Array<Vector3>|boolean} List of all intersections in plane's space, or false if invalid input provided.
   *
   * @example
   ...
   *
   */
  static aabbPlane(aabb: AABB, plane: Plane): Vector3[] | false {
    //
   ...
   */
  static rayPlane(ray: Ray, plane: Plane): Vector3 | null {
   ...
   * @return {Array}
   */
  static rayBox(ray: Ray, box: Box): Vector3[] {
   ...
   * @param {*} intersections
   */
  static rayPlaneInBBox(ray: Ray, planeAABB: Plane, bbox: any, intersections: Vector3[]) {
   ...
   */
  static findIntersection(myintersection: Vector3) {
    return function found(element: Vector3, index: number, array: Vector3[]) {
      if (
   ...
   * @return {Boolean}
   */
  static inBBox(point: Vector3, bbox: any): boolean {
    //
  ...
  static posdir(position: Vector3, direction: Vector3): Ray {
    return { position, direction };
  }

  static validatePlane(plane: Plane): boolean {
    //
    if (plane === null) {
  ...
  static validateAabb(aabb: AABB): boolean {
    //
    if (aabb === null) {
