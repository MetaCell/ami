 ...
 */
const helpersStack = (three: typeof THREE = window.THREE!) => {
  ...
  return class HelpersStack extends Constructor {
     ...
     * @type {boolean}
     */
    set outOfBounds(outOfBounds: boolean) {
     ...
     * @type {number}
     */
    set orientationMaxIndex(orientationMaxIndex: number) {
     ...
     * @type {number}
     */
    set orientationSpacing(orientationSpacing: number) {
     ...
     */
    _prepareSliceIndex(indices: THREE.Vector3): number {
      let index = 0;
      switch (this._orientation) {
        case 0:
     ...
     * @returns {number} Slice index according to current orientation.
     */
    _prepareSlicePosition(rPosition: THREE.Vector3, index: number): THREE.Vector3 {
     ...
     */
    _prepareDirection(orientation: number): THREE.Vector3 {
