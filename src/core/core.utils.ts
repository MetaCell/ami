import { Box3, BufferGeometry, Matrix4, Raycaster, Triangle, Vector2, Vector3 } from 'three';
import Validators from './core.validators';

interface OrderedPoint extends Vector3 {
  direction: Vector3;
...
export default class CoreUtils {
   ...
   *
   */
  static bbox(center: Vector3, halfDimensions: Vector3): { min: Vector3; max: Vector3 } | false {
  ...
  static parseUrl(url: string) {
    ...
    const data = {
      filename: parsedUrl.searchParams.get('filename') as string | null,
      extension: '',
  ...
  static ijk2LPS(
    ...
    spacing: Vector3,
    origin: Vector3,
    registrationMatrix: Matrix4 = new Matrix4()
  ): Matrix4 {
   ...
   * @return {*}
   */
  static aabb2LPS(xCos: Vector3, yCos: Vector3, zCos: Vector3, origin: Vector3): Matrix4 {
   ...
   * @return {*}
   */
  static getPixelData(stack: any, coordinate: Vector3): number | null {
   ...
   * @return {*}
   */
  static setPixelData(stack: any, coordinate: Vector3, value: number) {
    if (coordinate.z >= 0 && coordinate.z < stack._frame.length) {
   ...
   * @return {*}
   */
  static rescaleSlopeIntercept(value: number, slope: number, intercept: number): number {
   ...
   */
  static centerOfMass(points: Vector3[]): Vector3 {
   ...
   */
  static orderIntersections(points: Vector3[], direction: Vector3): OrderedPoint[] {
    ...
    for (let j = 0; j < points.length; j++) {
      const point = new Vector3(points[j].x, points[j].y, points[j].z) as OrderedPoint;
    ...
    for (let x = min.x; x <= max.x; x++) {
      for (let y = min.y; y <= max.y; y++) {
        rayCaster.setFromCamera(
          new Vector2((x / offsetWidth) * 2 - 1, -(y / offsetHeight) * 2 + 1),
          camera
        );
   ...
   * @returns {Number}
   */
  static getGeometryArea(geometry: BufferGeometry): number {
  ...
  static stringToNumber(numberAsString: string): number {
    ...
    if (number !== number) {
      ...
      if (number !== number) {
        console.error(
          `String could not be converted to number (${numberAsString}). Setting value to "1.0".`
        );
        number = 1.0;
      }
    }
