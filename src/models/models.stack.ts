...
export default class ModelsStack extends ModelsBase {
  ...
  private _ijk2LPS: Matrix4 | null;
  private _lps2IJK: Matrix4 | null;

  private _aabb2LPS: Matrix4 | null;
  private _lps2AABB: Matrix4 | null;

  //
  // IJK dimensions
  private _dimensionsIJK: Vector3 | null;
  private _halfDimensionsIJK: Vector3 | null;
   ...
   * @param {*} stopVoxel
   */
  _packTo8Bits(channels: number, frame: any[], textureSize: number, startVoxel: number, stopVoxel: number) {
    const packed: { textureType: any; data: any } = {
      textureType: null,
      data: null,
    };
  ...
  _arrayToVector3(array: number[], index: number): Vector3 {
    return new Vector3(array[index], array[index + 1], array[index + 2]);
  }

  _orderFrameOnDimensionIndicesArraySort(a: any, b: any): number {
    if (
  ...
  _computeDistanceArrayMap(normal: Vector3, frame: any) {
  ...
  set numberOfChannels(numberOfChannels: number) {
  ...
  set packedPerPixel(packedPerPixel: number) {
  ...
  set dimensionsIJK(dimensionsIJK: Vector3 | null) {
  ...
  set halfDimensionsIJK(halfDimensionsIJK: Vector3 | null) {
  ...
  set regMatrix(regMatrix: Matrix4) {
  ...
  set ijk2LPS(ijk2LPS: Matrix4 | null) {
  ...
  set lps2IJK(lps2IJK: Matrix4 | null) {
  ...
  set lps2AABB(lps2AABB: Matrix4 | null) {
  ...
  set textureUnits(textureUnits: number) {
  ...
  set bitsAllocated(bitsAllocated: number) {
  ...
  set rescaleSlope(rescaleSlope: number) {
  ...
  set rescaleIntercept(rescaleIntercept: number) {
  ...
  set pixelType(pixelType: number) {
  ...
  set pixelRepresentation(pixelRepresentation: number) {
  ...
  set rightHanded(rightHanded: boolean) {
   ...
   * @return {*}
   */
  static value(stack: any, coordinate: Vector3) {
    window.console.warn(
   ...
   * @return {*}
   */
  static valueRescaleSlopeIntercept(value: number, slope: number, intercept: number): number {
    window.console.warn(
   ...
   * @return {*}
   */
  static worldToData(stack: any, worldCoordinates: Vector3): Vector3 {
    window.console.warn(
