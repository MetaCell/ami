...
const camerasOrthographic = (three: typeof THREE = window.THREE!) => {
  if (three === undefined || three.OrthographicCamera === undefined) {
    return null;
  }

  const Constructor = three.OrthographicCamera;
  return class CamerasOrthographic extends Constructor {
    _front: THREE.Vector3 | null;
    _back: THREE.Vector3 | null;

    _directions: THREE.Vector3[];
    ...
    _right: THREE.Vector3 | null;
    _up: THREE.Vector3 | null;
    _direction: THREE.Vector3 | null;

    _controls: any;
    _box: any;
    _canvas: { width: number | null; height: number | null };
     ...
     */
    init(xCosine: THREE.Vector3, yCosine: THREE.Vector3, zCosine: THREE.Vector3, controls: any, box: any, canvas: any) {
      // DEPRECATION NOTICE
      window.console.warn(
    ...
    fitBox(direction = 0, factor = 1.5) {
      ...
      //
      let zoom: number | false = 1;

      // update zoom
      switch (direction) {
        case 0:
          zoom = factor * (this._computeZoom(this._canvas.width, this._right!) || 0);
          break;
        case 1:
          zoom = factor * (this._computeZoom(this._canvas.height, this._up!) || 0);
          break;
        case 2:
          zoom =
            factor *
            Math.min(
              this._computeZoom(this._canvas.width, this._right!) as number,
              this._computeZoom(this._canvas.height, this._up!) as number
            );
          break;
        default:
          break;
    ...
    _adjustTopDirection(horizontalDirection: THREE.Vector3, verticalDirection: THREE.Vector3) {
    ...
    _getMaxIndex(vector: THREE.Vector3): number {
      // init with X value
    ...
    _findMaxIndex(directions: THREE.Vector3[], target: number): number | undefined {
    ...
    _getMaxIndices(directions: THREE.Vector3[]): number[] {
      const indices = [];
    ...
    _orderIntersections(intersections: THREE.Vector3[], direction: THREE.Vector3): THREE.Vector3[] {
    ...
    _oppositePosition(position: THREE.Vector3): THREE.Vector3 {
    ...
    _computeZoom(dimension: number | null, direction: THREE.Vector3): number | false {
    ...
    _updatePositionAndTarget(position: THREE.Vector3, target: THREE.Vector3) {
      // position
    ...
    _vector2Label(direction: THREE.Vector3): string {
    ...
    set canvas(canvas: { width: number | null; height: number | null }) {
      this._canvas = canvas;
      this._updateCanvas();
    }
    ...
    set directions(directions: THREE.Vector3[]) {
    ...
    set convention(convention: string) {
    ...
    set directionsLabel(directionsLabel: string[]) {
    ...
    set stackOrientation(stackOrientation: number) {
