...
const helpersVolumeRendering = (three: typeof THREE = window.THREE!) => {
  if (three === undefined || three.Object3D === undefined) {
    return null;
  }

  const Constructor = helpersMaterialMixin(three)!;
  return class HelpersVolumeRendering extends Constructor {
    _geometry: THREE.BoxGeometry | null;
