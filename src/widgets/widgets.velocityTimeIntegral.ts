...
const widgetsVelocityTimeIntegral = (three = window.THREE) => {
  ...
  return class WidgetsvelocityTimeIntegral extends Constructor {
    ...
    finalize() {
      ...
      const WidgetsHandle = widgetsHandleFactory(three);
      const params: { hideHandleMesh: boolean; worldPosition?: THREE.Vector3 } = {
    ...
    updateMesh() {
      ...
      const _points = this._handles.map(h => h.worldPosition);
      _points.push(this._handles[0].worldPosition);
      this._geometry = new three.BufferGeometry().setFromPoints(_points);
    ...
    updateDOMContent(clear: boolean = false) {
...
export default widgetsVelocityTimeIntegral(THREE);
